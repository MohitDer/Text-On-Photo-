...
class FilterAdapter(private val mFilterListener: FilterListener, Context:Context): RecyclerView.Adapter<FilterAdapter.ViewHolder>() {
    ...
    class ViewHolder(itemView:View):RecyclerView.ViewHolder(itemView) {
        ...
        val iv_filter:ImageView = itemView.findViewById(R.id.iv_filter);
        val cv_filter:CardView = itemView.findViewById(R.id.cv_filter)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        Glide.with(context).asBitmap().load(fromAsset).into(holder.iv_filter)
        holder.cv_filter.setOnClickListener {
            mFilterListener.onFilterSelected(
                mPairList[position].second
            )
        }
    }
